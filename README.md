...
| `/symbiont:health` | Три ответа: соблюдаются ли правила прямо сейчас, куда всё дрейфует относительно прошлых снимков и куда раз за разом ложатся починки — файлы, в которые bug-fix возвращается снова и снова. Последнее даёт кандидатов на рефакторинг, выбранных данными, а не ощущением. Плюс устаревание: правила, у которых основание могло уйти из-под ног (зона удалена, срок перепроверки вышел) — кандидаты на вытеснение, названные до того, как замер их отзовёт |
...
| `/symbiont:mute` | Сказать, что выведенное правило здесь вводит в заблуждение: оно перестаёт подаваться в сводку и судиться гейтом, а история остаётся целой. Фраза из правила (или его ключ) глушит ровно одно; при нескольких совпадениях — список и просьба уточнить. `list` показывает приглушённое, `undo` возвращает. Ничего не удаляется: журнал append-only, в `passport_conventions` правило остаётся с пометкой |
...
| `/symbiont:health` | Three answers: are the rules being followed right now, where is everything drifting compared to earlier snapshots, and where do repairs keep landing — the files bug fixes return to again and again. The last one gives refactoring candidates chosen by data, not by feeling. Plus staleness: rules whose ground may have shifted (a zone deleted, a re-check date passed) — supersession candidates named before a measurement retires them |
...
| `/symbiont:mute` | Say that a derived rule is misleading here: it stops being delivered in the summary and enforced by the gate, while its history stays intact. A phrase from the rule (or its key) mutes exactly one; several matches → a list and a request to refine. `list` shows what is muted, `undo` brings it back. Nothing is deleted: the journal is append-only, and `passport_conventions` keeps the rule with a mark |
