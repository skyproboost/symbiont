...
var astSupported = (ext) => (ext in EXT_LANG);
var IS_TRY = /^try/;
var IS_CATCH = /catch|except_clause|rescue/;
var IS_THROW = /^(throw|raise)/;
var IS_FN = /^(arrow_function|function_declaration|function_definition|function_expression|method_definition|method_declaration|func_literal|function_item)$/;
var IS_CLASS = /^class_(declaration|definition|specifier)/;
var IS_RETURN = /^return/;
var initDone = false;
...
async function loadLanguage(name2) {
  if (languages.has(name2))
    return languages.get(name2);
  const path = join(wasmDir(), `tree-sitter-${name2}.wasm`);
  if (!existsSync(path)) {
    languages.set(name2, null);
    return null;
  }
  try {
    if (!initDone) {
...
function collect(node, m) {
  ...
  if (IS_TRY.test(t))
    ...
    if (body2 && body2.namedChildCount === 0)
      m.emptyCatch++;
    if (hasDescendant(node, IS_RETURN))
      m.catchWithReturn++;
    if (hasDescendant(node, IS_THROW))
      m.catchWithRethrow++;
  } else if (IS_THROW.test(t))
    m.throwCount++;
  else if (IS_FN.test(t)) {
    m.fnTotal++;
    if (node.text.startsWith("async"))
      m.fnAsync++;
  } else if (IS_CLASS.test(t))
    m.classCount++;
  for (let i2 = 0;i2 < node.namedChildCount; i2++)
    collect(node.namedChild(i2), m);
}
function hasDescendant(node, re) {
  for (let i2 = 0;i2 < node.namedChildCount; i2++) {
    const c = node.namedChild(i2);
    if (re.test(c.type))
      return true;
    if (hasDescendant(c, re))
      return true;
  }
  return false;
}
