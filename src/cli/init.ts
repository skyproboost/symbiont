...
try {
  // Полный пересчёт снимает и ранний срез слоя 2 (learn_meta.layer2_material):
  // явное «заново» главнее экономии — иначе вербализация молча пропустилась бы
  // как «материал не менялся», хотя владелец прямо попросил всё пересчитать.
  if (full) {
    try {
      db.run("DELETE FROM learn_meta WHERE key='layer2_material'")
    } catch {
      /* таблицы ещё нет — срезать нечего */
    }
  }
