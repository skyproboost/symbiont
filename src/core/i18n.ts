 ...
 *      проектах владельца);
 *   8) русский — умолчание, когда ни один признак не сработал.
 *
 * ПОЧЕМУ УМОЛЧАНИЕ РУССКОЕ. Раньше здесь стоял английский — «плагин публичный,
 * человек, о котором ничего не известно, скорее говорит по-английски». Довод
 * верен для читателя README на GitHub и неверен для того, кто плагин запустил.
 * Умолчание — это не догадка о мире, а ответ на вопрос «что показать, когда
 * ПРИЗНАКОВ НЕТ», и цена ошибки здесь несимметрична: русскую строку в английской
 * сессии видно сразу и она чинится одной командой, а английская сводка у
 * русскоязычного владельца выглядит не ошибкой, а замыслом, и живёт годами.
 * Английский остаётся полноправным: его выбирают вслух (`/symbiont:lang en`) или
 * он выигрывает по наблюдениям — лестница выше не тронута.
 *
...
/**
 * Русский — умолчание процесса (обоснование в заголовке модуля). Переменная
 * окружения действует СРАЗУ при загрузке модуля, не дожидаясь initLang: иначе
 * строка, отрисованная до инициализации, ушла бы на другом языке.
 */
...
function decideFrom(state: LangState, projectRoot: string | null): { lang: Lang; source: LangSource } {
  ...
  // Признаков не нашлось. Умолчание русское — см. заголовок модуля: ошибка в эту
  // сторону видна владельцу сразу, ошибка в обратную выглядит замыслом.
