...
export function factBasis(fact: { source?: string; positive: number; total: number; prevalence: number }): string {
  ...
  // Пустой класс контрпримеров — это ОТСУТСТВИЕ явления, а не соблюдённая
  // норма, и говорить о нём надо иначе. «1016 из 1016 (100%)» читается как
  // измеренная дисциплина и стоит в строке рядом с «139 из 140 (99%)», где
  // правило действительно один раз нарушили: на глаз эти два основания
  // неразличимы, хотя означают разное. Правило со 100% нельзя нарушить, потому
  // что альтернатива в проекте ни разу не встретилась.
  //
  // Отвергнут вариант «не показывать такие факты вовсе» (просьба из разбора
  // паспорта): именно они и есть самые полезные для гейта — «var не
  // используется» ловится чаще всех прочих законов, и в репозитории без
  // единого var этот закон исчез бы вместе с проверкой. Правда о числе
  // наблюдений дешевле потери принуждения.
