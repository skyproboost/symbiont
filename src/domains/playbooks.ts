/**
 * Доменные плейбуки: проактивная экспертиза на направление (концепт §4.1).
 * «Вижу БД → вот как она должна быть устроена здесь топ-уровнем; вижу nginx →
 * вот топ-конфиг; вижу фронт → вот его планка». Заземлено на актуальный (2026)
 * ресёрч официальных источников (Postgres/nginx/Node/web.dev/Let's Encrypt).
 *
 * Данные, не логика (ноль LLM-токенов): чек-листы + пороги + анти-паттерны +
 * источник на направление. Активируются детектом стека (stack.ts) и при
 * касании файла направления (PostToolUse), подаются в /sym-elevate.
 */

export interface Playbook {
  /** Имя направления — совпадает с detectStack().domains или именем инфры/фреймворка. */
  domain: string
  /** Триггеры: имена из stack (domains/infra/frameworks), при которых плейбук активен. */
  triggers: string[]
  /** Топ-маркеры качества направления (что делает его топ-1). */
  checklist: string[]
  /** Объективные пороги с числами, где есть. */
  thresholds?: string[]
  /** Анти-паттерны «от обратного» (что ломают чаще всего). */
  pitfalls: string[]
  source: string
}
...
/** Плейбуки, активные для обнаруженного стека (по триггерам). */
export function playbooksFor(signals: { frameworks: string[]; infra: string[]; domains: string[] }): Playbook[] {
...
/** Компактный срез плейбука для подачи (PostToolUse при касании файла направления). */
export function renderPlaybookBrief(p: Playbook): string {
  const lines = [`Symbiont · плейбук «${p.domain}» (топ-уровень здесь):`]
  for (const c of p.checklist.slice(0, 6)) lines.push(`- ${c}`)
  if (p.thresholds && p.thresholds.length > 0) lines.push(`пороги: ${p.thresholds.join(' · ')}`)
