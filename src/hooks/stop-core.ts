...
export function handleStop(input: StopInput, dataRoot: string): StopOutput {
  try {
    ...
    try {
      ...
      const parallelLine =
        parallel > 0 && unattributed.length > 0
          ? `- параллельных сессий: ${parallel} · ${unattributed.length} изменённых файлов не отнесены к этой сессии — авторство не подтверждено (${unattributed.slice(0, 3).join(', ')}${unattributed.length > 3 ? ', …' : ''})`
          : ''
