...
/**
 * Порог присутствия языка: ниже него конвенции этого языка не становятся
 * законами репозитория. Доля — чтобы малый язык молчал в большом проекте;
 * абсолютное число — чтобы сотня PHP-файлов в трёхтысячном репозитории
 * говорила, хотя её доля мала. Достаточно ОДНОГО из двух условий.
 */
const MIN_LANG_SHARE = 0.05
const MIN_LANG_FILES = 30

/**
 * Есть ли язык в проекте настолько, чтобы его конвенции считались конвенциями
 * проекта. Считаем ФАЙЛЫ (extHist знает их поимённо по видам), а не строки:
 * один сгенерированный дамп раздувает строки, не прибавляя решений автора.
 */
function languagePresent(agg: Aggregate, exts: string[]): boolean {
  let files = 0
  for (const ext of exts) files += agg.extHist[ext] ?? 0
  if (files === 0) return false
  if (files >= MIN_LANG_FILES) return true
...
export function deriveFacts(agg: Aggregate): Fact[] {
  ...
  for (const axis of AXES) {
    ...
    // Язык должен быть языком ПРОЕКТА, а не случайным гостем в нём. Наблюдений
    // ось набирает с файлов, а не с проекта: шесть PHP-файлов (два из них —
    // одноразовые пробники) дали 140 наблюдений по массивам и вышли в сводку
    // Python-репозитория законом «массивы — короткий синтаксис []» — рядом с
    // отступами, намайненными со всей кодовой базы, и неотличимо от них.
    // Мера присутствия — файлы, а не наблюдения: один плотный файл легко
    // перевешивает по наблюдениям сотню чужих, но проектом от этого не станет.
    if (!languagePresent(agg, axis.exts)) continue
