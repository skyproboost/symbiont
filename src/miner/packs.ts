...
/**
 * Тот же проход для ОДНОЙ строки, вырванной из файла, — строки диффа. Отдельной
 * функцией из-за PHP: его файл начинается вне кода, и строка без `<?php` целиком
 * считалась бы разметкой, то есть пустой. Многострочные строки и комментарии
 * построчно не отследить — это цена работы по диффу, а не по файлу; одиночная
 * строка внутри блочного комментария остаётся на совести вызывающего.
 */
export function codeOnlyLine(line: string, ext: string): string {
